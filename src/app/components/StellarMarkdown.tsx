...
const preprocessContent = (content: string) => {
    ...
    newContent = newContent.replace(
        ...
        (match, args, body) => {
            ...
            return `<div class="tag-plugin colorful note" color="${color}">${title ? `<div class="title">${title}</div>` : ''}<div class="body">${body}</div></div>`;
        });
    ...
    newContent = newContent.replace(
        ...
        (match, args, body) => {
            ...
            return `<details class="tag-plugin colorful folding" ${color ? `color="${color}"` : ''} ${child ? `child="${child}"` : ''} ${isOpen ? 'open' : ''}><summary>${title}</summary><div class="body">${body}</div></details>`;
        }
    );
        ...
        (match, args, body) => {
            ...
            if (body.includes('<!-- node')) {
                ...
                for (let i = 1; i < nodes.length; i += 2) {
                    ...
                    html += `<div class="timenode" index="${(i - 1) / 2}"><div class="header">${header ? `<span>${header}</span>` : ''}</div><div class="body fs14">${content}</div></div>`;
                }
            } else {
                ...
                items.forEach((item: string, index: number) => {
                    ...
                    html += `<div class="timenode" index="${index}"><div class="header">${header ? `<span>${header}</span>` : ''}</div><div class="body fs14">${content}</div></div>`;
                });
            }
            html += `</div>`;
            return html;
    ...
    newContent = newContent.replace(
        ...
        (match, args, body) => {
            ...
            for (let i = 1; i < parts.length; i += 2) {
                ...
                nav += `<div class="tab${activeClass}" data-href="${id}"><a href="javascript:;">${title}</a></div>`;
    ...
    newContent = newContent.replace(
        ...
        (match, args) => {
            ...
            return `<div class="tag-plugin link dis-select"><a class="link-card ${isRich}" title="${props.title}" href="${props.url}" target="_blank" rel="external nofollow noopener noreferrer" cardlink>${props.desc ? `<div class="top">${props.icon ? `<div class="lazy img" style="background-image:url(${props.icon})"></div>` : ''}<span class="cap link footnote">${props.url}</span></div><div class="bottom"><span class="title">${props.title}</span><span class="cap desc footnote">${props.desc}</span></div>` : `<div class="left"><span class="title">${props.title}</span><span class="cap link footnote">${props.url}</span></div><div class="right">${props.icon ? `<div class="lazy img" style="background-image:url(${props.icon})"></div>` : ''}</div>`}</a></div>`;
        }
    );
    ...
    newContent = newContent.replace(
        ...
        (match, args) => {
            ...
            return `<a class="tag-plugin colorful button" color="${props.color}" size="${props.size}" title="${text}" href="${url}">${props.icon ? `<img src="${props.icon}" style="width:1.2em;height:1.2em;vertical-align:middle;margin-right:4px;"/>` : ''}<span>${text}</span></a>`;
        }
    );
    ...
    newContent = newContent.replace(
        ...
        (match, args, body) => {
            ...
            return `<div class="tag-plugin about">${props.avatar ? `<div class="about-header"><div class="avatar"><img src="${props.avatar}" style="${props.border ? `border-radius:${props.border};` : ''}" height="${props.height || '80px'}"/></div></div>` : ''}<div class="about-body fs14">${body}</div></div>`;
        }
    );
...
const Scripts = () => {
    useEffect(() => {
        const handleTabClick = (e: MouseEvent) => {
            ...
            const tabEl = target.closest('.tag-plugin.tabs .nav-tabs .tab');
            if (tabEl) {
                e.preventDefault();
                const id = tabEl.getAttribute('data-href');
                if (!id) return;

                const wrapper = tabEl.closest('.tag-plugin.tabs');
                if (!wrapper) return;

                wrapper.querySelectorAll('.nav-tabs .tab').forEach((el: Element) => {
                    const elHref = el.getAttribute('data-href');
                    if (elHref === id) el.classList.add('active');
